...
for node in result_info['nodepools'] :
    #print(type(i))
    print (node['usage'])
    #Percentable disk utilization is 100 - (percentage disk available)
    available_bytes = float(node['usage']['free_bytes'])
    total_bytes = float(node['usage']['total_bytes'])
    print ('%Usage: ' + "{0:.4f}".format( 100 - (available_bytes*100/total_bytes))+"%")
